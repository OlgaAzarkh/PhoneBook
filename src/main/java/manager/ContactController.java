...
public class ContactController implements BaseAPI {
    ...
    public void login() {
        ...
        if (response.getStatusCode() == 200) {
            ...
            System.out.println("Smth went wrong -> " + response.getStatusCode());
        }
    }
    ...
    protected Response updateContact(Contact contact, TokenDto tokenDto) {
        return given()
                .log().all() //body, headers, cookies
                .body(contact)
                .baseUri(getProperty("login.properties", "baseUri"))
                .contentType(ContentType.JSON)
                .accept(ContentType.JSON)
